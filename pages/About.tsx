...
const About: React.FC = () => {
  ...
  useEffect(() => {
    let active = true;
    fetchSanity<AboutContent>(aboutQuery).then((data) => {
      if (data && active) {
        setContent(data);
      }
    });
      ...
      {/* Header */}
       <div className="bg-brand-dark py-24 text-center text-white relative overflow-hidden">
         ...
         <div className="relative z-10">
            <h1 className="text-5xl md:text-6xl font-display font-bold mb-4">{content.header.title}</h1>
            <p className="text-slate-300 text-lg uppercase tracking-widest font-semibold">{content.header.subtitle}</p>
         </div>
      </div>
          ...
          <div className="lg:w-1/2 flex flex-col gap-6">
            <div className="relative rounded-2xl overflow-hidden shadow-2xl group">
              ...
              <div className="absolute bottom-6 left-6 text-white">
                 <p className="font-display font-bold text-xl">{content.visual.cardTitle}</p>
                 <p className="text-slate-300 text-sm">{content.visual.cardSubtitle}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-brand-blue p-6 rounded-xl text-white text-center flex flex-col items-center justify-center shadow-lg">
                 ...
                 <span className="text-3xl font-display font-bold">{content.visual.stats[0].value}</span>
                 <span className="text-xs uppercase opacity-70 mt-1">{content.visual.stats[0].label}</span>
              </div>
              <div className="bg-brand-dark p-6 rounded-xl text-white text-center flex flex-col items-center justify-center shadow-lg">
                 ...
                 <span className="text-3xl font-display font-bold">{content.visual.stats[1].value}</span>
                 <span className="text-xs uppercase opacity-70 mt-1">{content.visual.stats[1].label}</span>
              </div>
            </div>
             ...
             <div className="bg-slate-50 border border-slate-100 p-8 rounded-xl relative overflow-hidden">
                ...
                <h3 className="font-display text-xl font-bold mb-2 relative z-10">{content.visual.promiseTitle}</h3>
                <p className="text-slate-600 text-sm relative z-10">{content.visual.promiseText}</p>
             </div>
          </div>
        </div>
      </div>
